...
var product = entry()
var id = product.field('DSLD ID')
if(id) {
  message("ID already set.")
}
else{
  var response = http().get(makeSearchURL(product))
...
if (data) {
  ...
  var ings = getIngredients(data, product)
  product.set('Main Ingredients', ings)
  setDosages(product, data["ingredientRows"])
  var othersData = data["otheringredients"]
  //message(JSON.stringify(othersData))
  if (othersData['ingredients'] !== null) {
    var others = getOthers(
      othersData, product 
    )
    product.set('Other Ingredients', others)
  }
  var company = setCompany(data, product)
}
